...
- **integers**: signed and unsigned
    ...
    - integer overflow when assigning values:
        ...
        - `b[0] = 127` is fine, but `a[0] = -128` isn't
        - `b[0] = -128` is fine, but `b[0] = -129` isn't
    - integer overflow when doing in-place integer math:
        - `a = np.zeros(5, dtype=np.uint8)`
        - `a += 1` is fine, but then another `a += 255` isn't
